                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('accounting.vouchers.store') }}" method="POST" id="voucherForm">
                                    ...
                                    {{-- Single hidden that always posts the selected Party --}}
                                    ...
                                    {{-- Voucher type pills --}}
                                    <div class="type-pills btn-group mb-3 flex-wrap" role="group"
                                    ...
                                    {{-- Payment / Receipt --}}
                                    <div id="section-payment-receipt" class="section-card mb-3" style="display:none;">
                                        <div class="section-title">
                                            <span class="badge bg-info">Payment / Receipt</span> Fill the instrument & party
                                            details
                                        </div>
                                        <div class="row g-3">
                                            <div class="col-lg-4 col-md-6">
                                                <label class="form-label">Party Ledger</label>
                                                {{-- UI-only select (no name); synced to hidden #party_ledger_id --}}
                                                <select id="pr_party_ledger" class="form-control">
                                                    <option value="">Select</option>
                                                    @foreach ($ledgers as $l)
                                                        <option value="{{ $l->id }}" @selected(old('party_ledger_id') == $l->id)>
                                                            {{ $l->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-lg-3 col-md-6" id="pr_cash_wrap" style="display:none;">
                                                <label class="form-label">Cash Ledger</label>
                                                <select id="pr_cash_ledger" name="cash_ledger_id" class="form-control">
                                                    <option value="">Select</option>
                                                    @foreach ($ledgers as $l)
                                                        <option value="{{ $l->id }}" @selected(old('cash_ledger_id') == $l->id)>
                                                            {{ $l->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>

                                            <div class="col-lg-3 col-md-6" id="pr_bank_wrap" style="display:none;">
                                                <label class="form-label">Bank Ledger</label>
                                                <select id="pr_bank_ledger" name="bank_ledger_id" class="form-control">
                                                    <option value="">Select</option>
                                                    @foreach ($ledgers as $l)
                                                        <option value="{{ $l->id }}" @selected(old('bank_ledger_id') == $l->id)>
                                                            {{ $l->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-lg-2 col-md-4">
                                                ...
                                                <input id="pr_amount" name="amount" type="number" step="0.01"
                                                    class="form-control" value="{{ old('amount') }}">
                                            </div>

                                            <div class="col-lg-2 col-md-4">
                                                <label class="form-label">Instrument No</label>
                                                <input id="pr_inst_no" name="instrument_no" class="form-control"
                                                    value="{{ old('instrument_no') }}">
                                            </div>

                                            <div class="col-lg-2 col-md-4">
                                                <label class="form-label">Instrument Date</label>
                                                <input id="pr_inst_date" name="instrument_date" type="date"
                                                    class="form-control" value="{{ old('instrument_date') }}">
                                            </div>

                                            <div class="col-12">
                                                <div class="form-check mt-2">
                                                    <input class="form-check-input" type="checkbox" id="pr_autobuild"
                                                        checked>
                                                    <label class="form-check-label" for="pr_autobuild">
                                                        Auto-build balanced lines from these fields
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div id="section-contra" class="section-card mb-3" style="display:none;">
                                        <div class="section-title">
                                            <span class="badge bg-warning text-dark">Contra</span> Move amount between
                                            ledgers
                                        </div>
                                        <div class="row g-3">
                                            <div class="col-md-5">
                                                <label class="form-label">From Ledger</label>
                                                <select id="ct_from" name="from_ledger_id" class="form-control">
                                                    <option value="">Select</option>
                                                    @foreach ($ledgers as $l)
                                                        <option value="{{ $l->id }}" @selected(old('from_ledger_id') == $l->id)>
                                                            {{ $l->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-5">
                                                ...
                                                <select id="ct_to" name="to_ledger_id" class="form-control">
                                                    <option value="">Select</option>
                                                    @foreach ($ledgers as $l)
                                                        <option value="{{ $l->id }}" @selected(old('to_ledger_id') == $l->id)>
                                                            {{ $l->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label">Amount</label>
                                                <input id="ct_amount" name="contra_amount" type="number" step="0.01"
                                                    class="form-control" value="{{ old('contra_amount') }}">
                                            </div>
                                            <div class="col-12">
                                                <div class="form-check mt-2">
                                                    <input class="form-check-input" type="checkbox" id="ct_autobuild"
                                                        checked>
                                                    <label class="form-check-label" for="ct_autobuild">Auto-build
                                                        lines</label>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div id="section-trade" class="section-card mb-3" style="display:none;">
                                        <div class="section-title">
                                            <span class="badge bg-primary">Sales / Purchase / Notes</span> Totals with live
                                            calculation
                                        </div>
                                        <div class="grid-2">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <input id="tr_tax" name="tax" type="number" step="0.01"
                                                    class="form-control" value="{{ old('tax', 0) }}">
                                            </div>
                                            <div>
                                                <label class="form-label">Grand Total</label>
                                                <input id="tr_grand" name="grand_total" type="number" step="0.01"
                                                    class="form-control" value="{{ old('grand_total', 0) }}" readonly>
                                            </div>
                                        </div>
                                        <div class="form-check mt-2">
                                            <input class="form-check-input" type="checkbox" id="tr_autobuild" checked>
                                            <label class="form-check-label" for="tr_autobuild">Auto-build lines from
                                                totals</label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="section-card">
                                        <div class="section-title">
                                            <span class="badge bg-secondary">Journal</span> Add line items (Dr/Cr)
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-2">
                                            <button type="button" id="addLine" class="btn btn-outline-primary">+ Add
                                                Line</button>
                                            <div class="d-flex align-items-center gap-2">
                                    ...
                                    {{-- Sticky footer for totals + submit --}}
                                    <div class="sticky-actions mt-3">
                                        <div class="d-flex flex-wrap align-items-center gap-3">
                                            ...
                                            <span id="stickyBadge" class="badge bg-secondary">Not Calculated</span>
                                        </div>
    ...
    <script>
        (function() {
            ...
            <tr class="line">
                ...
                <td><select name="lines[${idx}][dc]" class="form-control dc"><option>Dr</option><option>Cr</option></select></td>
            ...
            function autobuildTR() {
                ...
                if (t === 'Sales' || t === 'CreditNote') {
                    ...
                    $r0.find('.amount').val(amt.toFixed(2));
                    const $r1 = ensureRow(1, 'Cr');
                    $r1.find('.amount').val(amt.toFixed(2)); // user selects Sales ledger
                } else if (t === 'Purchase' || t === 'DebitNote') {
                    const $r0 = ensureRow(0, 'Dr');
                    $r0.find('.amount').val(amt.toFixed(2)); // user selects Purchase ledger
                    const $r1 = ensureRow(1, 'Cr');
            ...
            // ---------- Party hidden field sync ----------
            function activeType() {
                return $('#voucher_type').val();
            }
            ...
            $type.on('change', function() {
                ...
                togglePRMode();
                syncPartyHidden();
                // allow autobuild on type change only if no amounts present
                autobuildPR();
                autobuildCT();
                calcTradeGrand();
                autobuildTR();
            ...
            $('#btnSubmit').on('click', function(e) {
                syncPartyHidden(); // ensure hidden has latest value
                const dr = parseFloat($totalDr.val() || 0);
                const cr = parseFloat($totalCr.val() || 0);
            ...
            syncPills();
            togglePRMode();
            syncPartyHidden(); // load old() into hidden
            recalc();
            calcTradeGrand();
            // No autobuild on first paint to preserve old() values.
        })();
    </script>
@endsection
