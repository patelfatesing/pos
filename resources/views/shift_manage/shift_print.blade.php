...
<body>
    <div class="header">Shift Close Summary - {{ $branch_name ?? 'Shop' }}</div>
    ...
    <table class="table">
        <thead>
            <tr>
                <th>Denomination</th><th>Notes</th><th>x</th><th>Amount</th><th>=</th><th>Total</th>
            </tr>
        </thead>
        <tbody>
            @php $totalDenom = 0; @endphp
            @foreach($shiftcash ?? [] as $denom => $qty)
                @php $row = $denom * $qty; $totalDenom += $row; @endphp
                <tr>
        ...
        <tfoot>
            <tr class="highlight"><td colspan="5">Total</td><td>{{ format_inr($totalDenom) }}</td></tr>
        </tfoot>
    </table>

    <table class="table" style="margin-top:10px;">
        <tr><td><strong>System Cash Sales</strong></td><td class="text-end">{{ format_inr($totalDenom) }}</td></tr>
        <tr><td><strong>Total Cash Amount</strong></td><td class="text-end">{{ format_inr($categoryTotals['summary']['TOTAL'] ?? 0) }}</td></tr>
        <tr><td><strong>Closing Cash</strong></td><td class="text-end">{{ format_inr($closing_cash ?? 0) }}</td></tr>
        <tr><td><strong>Discrepancy Cash</strong></td><td class="text-end">{{ format_inr($cash_discrepancy ?? 0) }}</td></tr>
    </table>
</body>
</html>
