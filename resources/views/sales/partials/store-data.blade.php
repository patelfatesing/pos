                        ...
                        @if ($first->shift->status == 'completed')
                            <div class="d-flex align-items-center gap-3">
                                ...
                                @if (auth()->user()->role_id == 1)
                                    <button class="btn btn-sm btn-warning mt-2 mr-1"
                                        onclick="openSubAdminModal(
                                            ...
                                            // SUB ADMIN (REQUIRED)
                                            '{{ $status['sub_admin']['inv'] ?? '' }}',
                                            '{{ $status['sub_admin']['tra'] ?? '' }}',
                                            '{{ $status['sub_admin']['shift'] ?? '' }}'
                                        )">
                                        Sub Admin Verify
                                    </button>
                                @else
                                        ...
                                        <div class="d-flex align-items-center gap-4">
                                            ...
                                            <div class="verify-item text-center mr-2">
                                                ...
                                                <label class="verify-checkbox">
                                                    ...
                                                    <!-- ✅ HIDDEN CHECKBOX (IMPORTANT) -->
                                                    <input type="checkbox"
                                                        onchange="handleVerifyChange(this, 'sales', {{ $first->shift_id }}, 'sub_admin')"
                                                        {{ ($status['sub_admin']['inv'] ?? '') == 'verify' ? 'checked' : '' }}>
                                                    <!-- ✅ BUTTON UI -->
                                                    <div class="verify-single-btn 
                                                         {{ ($status['sub_admin']['inv'] ?? '') == 'verify' ? 'verified' : 'unverified' }}"
                                                        onclick="triggerVerifyBtn(event, this)">

                                                        <span class="icon">
                                                            {{ ($status['sub_admin']['inv'] ?? '') == 'verify' ? '✔' : '✖' }}
                                                        </span>
                                            ...
                                            <div class="verify-item text-center mr-2">
                                                <small class="text-muted d-block mb-1">
                                                    <a href="javascript:void(0)"
                                                        onclick="event.stopPropagation(); openTransferModal({{ $first->shift_id }}, {{ $first->branch->id }})">
                                                        Transfer
                                                    </a>
                                                </small>
                                                ...
                                                <label class="verify-checkbox">
                                                    ...
                                                    <input type="checkbox"
                                                        onchange="handleVerifyChange(this, 'transfer', {{ $first->shift_id }}, 'sub_admin')"
                                                        {{ ($status['sub_admin']['tra'] ?? '') == 'verify' ? 'checked' : '' }}>
                                                    <div class="verify-single-btn 
                                                          {{ ($status['sub_admin']['tra'] ?? '') == 'verify' ? 'verified' : 'unverified' }}"
                                                        onclick="triggerVerifyBtn(event, this)">

                                                        <span class="icon">
                                                            {{ ($status['sub_admin']['tra'] ?? '') == 'verify' ? '✔' : '✖' }}
                                                        </span>
                                            ...
                                            <div class="verify-item text-center">
                                                ...
                                                <label class="verify-checkbox">
                                                    <input type="checkbox"
                                                        onchange="handleVerifyChange(this, 'shift', {{ $first->shift_id }}, 'sub_admin')"
                                                        {{ ($status['sub_admin']['shift'] ?? '') == 'verify' ? 'checked' : '' }}>
                                                    <div class="verify-single-btn 
                                                        {{ ($status['sub_admin']['shift'] ?? '') == 'verify' ? 'verified' : 'unverified' }}"
                                                        onclick="triggerVerifyBtn(event, this)">

                                                        <span class="icon">
                                                            {{ ($status['sub_admin']['shift'] ?? '') == 'verify' ? '✔' : '✖' }}
                                                        </span>
